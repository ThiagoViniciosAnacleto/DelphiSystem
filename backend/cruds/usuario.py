...
from backend.models import Usuario, Role
from backend.schemas import UsuarioCreate, UsuarioUpdate
from backend.auth import gerar_hash_senha, validar_complexidade_senha


def criar_usuario(db: Session, dados: UsuarioCreate) -> Usuario:
    valida, motivo = validar_complexidade_senha(dados.senha)
    if not valida:
...
def buscar_usuario_por_id(db: Session, usuario_id: int) -> Usuario | None:
    return db.query(Usuario).options(joinedload(Usuario.role)).filter(Usuario.id == usuario_id, Usuario.ativo == True).first()


get_usuario = buscar_usuario_por_id
